...
SELECT id, length, width FROM rectangles;
1. **How many rectangles are there?**  17
   SELECT COUNT(*) FROM rectangles;
1. **What color is the widest rectangle?** #abcdef
SELECT color FROM rectangles ORDER BY width DESC LIMIT 1;
1. **What color is the tallest rectangle?** #ffffff
...
1. **Concatenate Course ID and name:**  
   ...
   SELECT CONCAT(course_id, ': ', course_name) AS course_info FROM courses;
